...
def is_safe_state(allocation, max_demand, available):
    ...
    Excludes processes with zero max demand (terminated processes).
    """
    ...
    # Mark processes with zero max demand as already finished (terminated)
    for i in range(num_processes):
        if all(max_demand[i][j] == 0 for j in range(num_resources)):
            finish[i] = True
            print(f"Process P{i} is terminated (Max Demand all 0), marked as finished.")
    
    print(f"Initial Available Resources (Work): {work}")
    
    # Step 3: Try to find a safe sequence
    count = sum(1 for f in finish if f)  # Count already finished processes
    while len(safe_sequence) < num_processes - count:
        progress = False
        for i in range(num_processes):
            if not finish[i] and all(need[i][j] <= work[j] for j in range(num_resources)):
                print(f"Process P{i} can proceed. Need: {need[i]}, Work: {work}")
...
def request_resources(process_id, request, allocation, max_demand, available):
    ...
    # Edge case: Check if process is terminated (max demand is all 0)
    if all(max_demand[process_id][j] == 0 for j in range(num_resources)):
        return False, "Error: Process is terminated."
