        ...
        with col1:
            if st.button("Terminate Process"):
                result = recovery.recover_from_deadlock(
                    st.session_state.allocation,
                    st.session_state.available,
                    st.session_state.max_demand,  # Pass max_demand to set it to 0 on termination
                    strategy="terminate"
                )
                if result:
                    ...
                    st.session_state.need = bankers.calculate_need(
                        st.session_state.allocation, st.session_state.max_demand)
                    # Check if system is safe after termination
                    safe, seq = bankers.is_safe_state(
                        st.session_state.allocation, st.session_state.max_demand, st.session_state.available)
                    if safe:
                        st.success(f"System is now in a safe state. Safe sequence: {seq}")
                    else:
                        st.error("System is still not in a safe state.")
                    # Display updated state
                    display_state(st.session_state.allocation, st.session_state.max_demand, 
                                  st.session_state.available, st.session_state.need)
                else:
        ...
        with col2:
            if st.button("Preempt Resources"):
                ...
                if result:
                    ...
                    st.session_state.need = bankers.calculate_need(
                        st.session_state.allocation, st.session_state.max_demand)
                    # Check if system is safe after preemption
                    safe, seq = bankers.is_safe_state(
                        st.session_state.allocation, st.session_state.max_demand, st.session_state.available)
                    if safe:
                        st.success(f"System is now in a safe state. Safe sequence: {seq}")
        ...
        with col3:
            if st.button("Rollback Process"):
                ...
                if result:
                    ...
                    st.session_state.need = bankers.calculate_need(
                        st.session_state.allocation, st.session_state.max_demand)
                    # Check if system is safe after rollback
                    safe, seq = bankers.is_safe_state(
                        st.session_state.allocation, st.session_state.max_demand, st.session_state.available)
                    if safe:
                        st.success(f"System is now in a safe state. Safe sequence: {seq}")
